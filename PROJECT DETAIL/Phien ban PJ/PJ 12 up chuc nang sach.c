					...
					case 2:  //thong tin giang vien
						while(1==1)
						{
							...
							else if (strcmp(choiTchr,"2.2") == 0) 
							{
								...
								deleteChar(mySchool[curIndexTeach].tchrName); // neu khong dung strcspn thi dung ham nay cung duoc 
...
void gotoXY(int x, int y) // ham di chuyen con tro den vi tri XY trong man hinh.
{
    COORD coord; // COORD la cau truc co san trong window.h chua hai truong X, y de bieu dien mot diem trong khong gian 2D
    coord.X = x;// gan gia tri x,y cho cac truong XY trong cau truc
    coord.Y = y;
    SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);
    // SetConsoleCursorPosition di chuyen con tro den vi tri duoc xac dinh. 
}
...
void startEffect()
{
	...
	system("pause"); // lenh tam dung cho den khi nhan phim bat ki
	printf("\033[H\033[J");
}
...
int signUp(User userList[], int *userCount) 
{
	...
	printf("\n\t\t\t\t\t[+] DA LUU THONG TIN NGUOI DUNG VAO FILE!\n");
	fclose(userFile);
