...
const int maxUser = 5; // quy dinh so tai khoan toi da
...
void writeCourseToFile(CourseInfo courses[], int curIndexCourse);
...
//ham tinh nang sach
void writeBookToFile(BookInfo bookList[], int curIndexBook);
							...
							if (strcmp(choiDoc,"3.1") == 0) 
							{
								while(1==1)
								{
									...
									else if(strcmp(choiCourse,"3.1b") == 0)
									{
										...
										writeCourseToFile(courseList, curIndexCourse);
									}
							...
							else if (strcmp(choiDoc,"3.2") == 0) 
							{
								...
								{	
									...
									else if(strcmp(choiBook,"3.2b") == 0)
									{
										...
										writeBookToFile(bookList, curIndexBook);
									}
...
void writeCourseToFile(CourseInfo courses[], int curIndexCourse) {
    FILE *courseFile = fopen("C:\\Users\\DELL\\OneDrive\\Documents\\C PROGRAMING\\PROJECT DETAIL\\file_va_luu_file\\CourseInfo.bin", "wb");
    ...
    fwrite(courses, sizeof(CourseInfo), curIndexCourse, courseFile);
    fclose(courseFile);
}
...
void writeBookToFile(BookInfo bookList[], int curIndexBook) {
    FILE *bookFile = fopen("C:\\Users\\DELL\\OneDrive\\Documents\\C PROGRAMING\\PROJECT DETAIL\\file_va_luu_file\\BookInfo.bin", "wb");
    ...
    fwrite(bookList, sizeof(BookInfo), curIndexBook, bookFile);
    fclose(bookFile);
}
