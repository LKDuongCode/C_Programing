#include<stdio.h> // van de 1: da luu thong tin nguoi dung vao file , chay lan 1 thi ok, nhung chay lan 2 khong dang ki ma dang nhap luon thi lai dang nhap that bai. 
				 // van de 2: khi dang ki xong, thong tin user duoc luu vao trong file, nhung chay lan 2 thi thong tin user1 bi xoa thay = user2, no khong them user 2 sau user 1.
#include<math.h>
#include<unistd.h>
#include<stdlib.h>
void startEffect()
...
int main()
{
	...
	while(1==1)
	{
		...
		{     // kiem tra dung sai xem nguoi dung dang nhap hay chua 
            ...
            switch (choiUser) 
			{
                case 1:
                    ...
                    FILE *userFile = fopen("C:\\Users\\DELL\\OneDrive\\Documents\\C PROGRAMING\\PROJECT DETAIL\\User.bin","r+b");
		...
		else // dang nhap thanh cong
		{
			...
			// Mo file luu du lieu 
			FILE *tchrFile1;
			tchrFile1 = fopen("C:\\Users\\DELL\\OneDrive\\Documents\\C PROGRAMING\\PROJECT DETAIL\\Thong tin giang vien.txt", "w");
			if (tchrFile1 != NULL) 
			{
	    		// Di chuyen con tro len dau file
	    		fseek(tchrFile1, 0, SEEK_SET);
	    		
	    		fprintf(tchrFile1, "| %-4s | %-15s | %-4s | %-25s | %-10s | %-15s | %-8s | %-3s |\n", "ID", "Ten", "Tuoi", "Email", "Dia chi", "So dien thoai", "MaNganh", "STA");
	    
	    		// ghi danh sach vao file
	    		for (int i = 0; i < curIndexTeach; i++) 
				{
					
	        		fprintf(tchrFile1, "| %-4d | %-15s | %-4d | %-25s | %-10s | %-15s | %-8d | %-3d |\n", mySchool[i].tchrCode, mySchool[i].tchrName, mySchool[i].tchrAge, mySchool[i].tchrEmail, mySchool[i].tchrAddress, mySchool[i].tchrPhone, mySchool[i].tchrMajor, mySchool[i].tchrStatus);
	    		}
					...
					case 2: // thong tin giang vien
						while(1==1)
						{
							...
							else if (strcmp(choiTchr,"2.2") == 0) 
							{
								...
								FILE *tchrFile1;
								tchrFile1 = fopen("C:\\Users\\DELL\\OneDrive\\Documents\\C PROGRAMING\\PROJECT DETAIL\\Thong tin giang vien.txt", "a");
								if (tchrFile1 != NULL) 
								{
		    						...
		    						// ghi danh sach vao file
		        						...
		        						fprintf(tchrFile1, "| %-4d | %-15s | %-4d | %-25s | %-10s | %-15s | %-8d | %-3d |\n", mySchool[curIndexTeach].tchrCode, mySchool[curIndexTeach].tchrName, mySchool[curIndexTeach].tchrAge, mySchool[curIndexTeach].tchrEmail, mySchool[curIndexTeach].tchrAddress, mySchool[curIndexTeach].tchrPhone, mySchool[curIndexTeach].tchrMajor, mySchool[curIndexTeach].tchrStatus);
