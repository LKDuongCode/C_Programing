	...
	do{
		...
		switch(choice){
			...
			case 6:
				...
				for(int i=0;i<n;i++){
					for(int j=0;j<m;j++){
						int isPrime = 0;// bien dem xac dinh so luong uoc so
						if(arr[i][j]>=2){
							for(int a=2; a<= sqrt(arr[i][j]);a++){
								if(arr[i][j]%a == 0){// neu so do chia het cho a thi tang 1
									isPrime = 1;
									break;
								}
							}
						...
						if(isPrime == 0){// sau khi tinh toan, neu so do co mot uoc thi khong phai so nguyen to, neu khong co uoc nao thi la so nguyen to
							printf("%d \t",arr[i][j]);
						}
					}
				}
